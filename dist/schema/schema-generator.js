...
const InvalidDirectiveError_1 = require("../errors/InvalidDirectiveError");
class SchemaGenerator {
    ...
    static getObjectTypeDefinitionNode(name, directiveMetas) {
    ...
    static getInputObjectTypeDefinitionNode(name, directiveMetas) {
        if (!directiveMetas || !directiveMetas.length) {
            return;
        }
        return {
    ...
    static getFieldDefinitionNode(name, directiveMetas) {
        if (!directiveMetas || !directiveMetas.length) {
            return;
        }
        return {
    ...
    static getInputValueDefinitionNode(name, directiveMetas) {
        if (!directiveMetas || !directiveMetas.length) {
            return;
        }
        return {
    ...
    static getDirectiveNodes(directive) {
        const { nameOrDefinition, args } = directive;
        if (nameOrDefinition === "") {
            throw new InvalidDirectiveError_1.InvalidDirectiveError("There must be exactly one directive defined for directive", directive);
        }
        ...
        let directives = [];
        try {
            const parsed = graphql_1.parse(`type String ${nameOrDefinition}`);
            ...
            throw new InvalidDirectiveError_1.InvalidDirectiveError("Error parsing directive", directive);
        }
